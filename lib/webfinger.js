...
var xrd2jrd = async function(str) {
    ...
    var parser = new xml2js.Parser({trim: true, normalize: true, emptyTag: function() { return {}; }});
    var parsed = await parser.parseStringPromise(str);
    var doc = parsed.XRD;
    var jrd = {}, i;
    ...
    if (doc.hasOwnProperty("Link")) {
        ...
        for (i = 0; i < doc.Link.length; i++) {
            jrd.links.push(getLink(doc.Link[i]));
        }
    }
    return jrd;
};
